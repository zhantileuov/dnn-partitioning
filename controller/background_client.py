...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Lightweight background Triton load generator.")
...
class SharedStats:
    ...
    def mark_sent(self) -> Optional[int]:
        with self._lock:
            self._sent += 1
            self._in_flight += 1
            return self._sent
...
class BackgroundWorker(threading.Thread):
    ...
    def _make_payload(self) -> np.ndarray:
        ...
        return np.random.random_sample(self.input_shape).astype(self.dtype, copy=False)
