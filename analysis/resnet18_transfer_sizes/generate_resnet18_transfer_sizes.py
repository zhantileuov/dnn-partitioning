...
# For ResNet18 with a 1x3x224x224 input, these are the activation shapes
...
RESNET18_PARTITIONS = [
    ...
    ("layer2.1", (1, 128, 28, 28)),
    ("layer3.0", (1, 256, 14, 14)),
    ("layer3.1", (1, 256, 14, 14)),
    ("layer4.0", (1, 512, 7, 7)),
    ("layer4.1", (1, 512, 7, 7)),
]
...
def tail_model_name(partition_point: str) -> str:
    return "resnet18_tail_after_{0}".format(partition_point.replace(".", "_"))


def build_rows() -> list[dict]:
    rows = []
    for partition_point, activation_shape in RESNET18_PARTITIONS:
...
def write_plot(path: Path, rows: list[dict]) -> None:
    ...
    ax.bar([pos - width / 2 for pos in positions], fp32_kb, width=width, label="FP32", color="#2E5BFF")
    ax.bar([pos + width / 2 for pos in positions], fp16_kb, width=width, label="FP16", color="#00A37A")

    ax.set_title("ResNet18 Split Transfer Size per Tail Model")
    ...
    for pos, value in zip(positions, fp32_kb):
        ax.text(pos - width / 2, value + max(fp32_kb) * 0.015, f"{value:.0f}", ha="center", va="bottom", fontsize=9)
...
def main() -> None:
    ...
    write_csv(output_dir / "resnet18_transfer_sizes.csv", rows)
    write_plot(output_dir / "resnet18_transfer_sizes_kb.png", rows)

    print("wrote:", output_dir / "resnet18_transfer_sizes.csv")
    print("wrote:", output_dir / "resnet18_transfer_sizes_kb.png")
