...
# For MobileNetV2 with a 1x3x224x224 input, these are the activation shapes
# produced by the client-side prefix and transferred to the Triton tail model.
MOBILENET_V2_PARTITIONS = [
    ("features.0", (1, 32, 112, 112)),
    ("features.1", (1, 16, 112, 112)),
    ("features.2", (1, 24, 56, 56)),
    ("features.3", (1, 24, 56, 56)),
    ("features.4", (1, 32, 28, 28)),
    ("features.5", (1, 32, 28, 28)),
    ("features.6", (1, 32, 28, 28)),
    ("features.7", (1, 64, 14, 14)),
    ("features.8", (1, 64, 14, 14)),
    ("features.9", (1, 64, 14, 14)),
    ("features.10", (1, 64, 14, 14)),
    ("features.11", (1, 96, 14, 14)),
    ("features.12", (1, 96, 14, 14)),
    ("features.13", (1, 96, 14, 14)),
    ("features.14", (1, 160, 7, 7)),
    ("features.15", (1, 160, 7, 7)),
    ("features.16", (1, 160, 7, 7)),
    ("features.17", (1, 320, 7, 7)),
    ("features.18", (1, 1280, 7, 7)),
    ("adaptive_avg_pool", (1, 1280, 1, 1)),
    ("flatten", (1, 1280)),
    ("classifier.0", (1, 1280)),
]
...
def tail_model_name(partition_point: str) -> str:
    return "mobilenet_v2_tail_after_{0}".format(partition_point.replace(".", "_"))


def build_rows() -> list[dict]:
    rows = []
    for partition_point, activation_shape in MOBILENET_V2_PARTITIONS:
...
def write_plot(path: Path, rows: list[dict]) -> None:
    ...
    ax.bar([pos - width / 2 for pos in positions], fp32_kb, width=width, label="FP32", color="#0B6E4F")
    ax.bar([pos + width / 2 for pos in positions], fp16_kb, width=width, label="FP16", color="#DB7C26")

    ax.set_title("MobileNetV2 Split Transfer Size per Tail Model")
...
def main() -> None:
    ...
    write_csv(output_dir / "mobilenet_v2_transfer_sizes.csv", rows)
    write_plot(output_dir / "mobilenet_v2_transfer_sizes_kb.png", rows)

    print("wrote:", output_dir / "mobilenet_v2_transfer_sizes.csv")
    print("wrote:", output_dir / "mobilenet_v2_transfer_sizes_kb.png")
