...
# For MobileNetV3-Small with a 1x3x224x224 input, these are the activation
# shapes produced by the client-side prefix and transferred to the Triton tail
# model.
MOBILENET_V3_SMALL_PARTITIONS = [
    ("features.0", (1, 16, 112, 112)),
    ("features.1", (1, 16, 56, 56)),
    ("features.2", (1, 24, 28, 28)),
    ("features.3", (1, 24, 28, 28)),
    ("features.4", (1, 40, 14, 14)),
    ("features.5", (1, 40, 14, 14)),
    ("features.6", (1, 40, 14, 14)),
    ("features.7", (1, 48, 14, 14)),
    ("features.8", (1, 48, 14, 14)),
    ("features.9", (1, 96, 7, 7)),
    ("features.10", (1, 96, 7, 7)),
    ("features.11", (1, 96, 7, 7)),
    ("features.12", (1, 576, 7, 7)),
    ("avgpool", (1, 576, 1, 1)),
    ("flatten", (1, 576)),
    ("classifier.0", (1, 1024)),
    ("classifier.1", (1, 1024)),
...
def tail_model_name(partition_point: str) -> str:
    return "mobilenet_v3_small_tail_after_{0}".format(partition_point.replace(".", "_"))


def build_rows() -> list[dict]:
    rows = []
    for partition_point, activation_shape in MOBILENET_V3_SMALL_PARTITIONS:
...
def write_plot(path: Path, rows: list[dict]) -> None:
    ...
    ax.bar([pos - width / 2 for pos in positions], fp32_kb, width=width, label="FP32", color="#8D6A9F")
    ax.bar([pos + width / 2 for pos in positions], fp16_kb, width=width, label="FP16", color="#2A7F62")

    ax.set_title("MobileNetV3 Small Split Transfer Size per Tail Model")
...
def main() -> None:
    ...
    write_csv(output_dir / "mobilenet_v3_small_transfer_sizes.csv", rows)
    write_plot(output_dir / "mobilenet_v3_small_transfer_sizes_kb.png", rows)

    print("wrote:", output_dir / "mobilenet_v3_small_transfer_sizes.csv")
    print("wrote:", output_dir / "mobilenet_v3_small_transfer_sizes_kb.png")
